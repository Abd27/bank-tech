...
describe Statement do
  ...
  it 'takes transactions upon initialize and saves in entries array' do
    expect(subject.entries).to include("#{Time.now.strftime('%d/%m/%Y')} || 0.00 || 50.00 || 50.00")
  end

  describe '.display' do
    it 'prints the statment on screen' do
      example = "date || credit || debit || balance\n#{Time.now.strftime('%d/%m/%Y')} || 0.00 || 50.00 || 50.00\n#{Time.now.strftime('%d/%m/%Y')} || 100.00 || 0.00 || 100.00\n"
